...
const Reviews = (): JSX.Element => {
    ...
    return (
        <Box maxWidth={600} width={1} margin={'0 auto'}>
            <Typography variant={'h4'} align={'center'} sx={{ fontWeight: 700, marginBottom: 4, color: 'common.white' }}>
                Trusted by the world’s most innovative businesses – big and small
            </Typography>
            <Slider {...sliderOpts}>
                {mock.map((item, i) => (
                    <Box key={i} paddingX={4}>
                        <Typography align={'center'} variant={'h6'} sx={{ color: 'common.white', fontStyle: 'italic' }}>
                            {item.feedback}
                        </Typography>
                    </Box>
                ))}
